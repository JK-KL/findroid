...
constructor(
    ...
    private var allPeople: List<FindroidPerson> = emptyList()
    ...
    private suspend fun getPeople(item: FindroidShow): List<FindroidPerson> {
        val perple: List<FindroidPerson>
        withContext(Dispatchers.Default) {
            perple = item.people.map { it.toFindroidPerson(jellyfinRepository.getBaseUrl()) }
        }
        return perple
    }
